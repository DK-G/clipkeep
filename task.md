...
2. [~] 計測/柱1: GSC カバレッジ取得（indexed 実数化）。`growth:review` に URL Inspection 由来の indexed カウント＋除外理由を追加 ← **計測部分は完了（2026-06-16, ver 未デプロイ=ローカル計測ツール）**。全件スイープ実測 indexed=**20**/500、**477=「URL is unknown to Google」**（発見/クロール遅延が律速と確定）。**残: sitemap 再送信は書込スコープ未付与で自律不可 → GSC UI で手動再送信（要人間操作）**。詳細 docs/ops/daily/2026-06-16.md
...
## 更新メモ
- 2026-06-16: 日次ループ。**バックログ #2 の計測部分（indexed 実数化＋除外理由の一次データ）を完遂**。新規 `scripts/fetch-gsc-index-coverage.mjs`（URL Inspection API, read-only スコープで可）を追加し `growth:review` に **📚 INDEX COVERAGE** ブロックを新設（`analytics:gsc:coverage` スクリプト追加、`run-growth-review` に組込、日次は既定 100 サンプル / 週次は `GSC_INSPECT_LIMIT=0` で全件）。**初の indexed 実数（全件 500 スイープ）: indexed=20 / Crawled-not-indexed=3 / "URL is unknown to Google"=477 / canonical 不一致=0**。律速は「コンテンツ品質」でも「crawled but not indexed」でもなく**発見（discovery/crawl）**＝06-12 sitemap 刷新後 Google が新 URL 群を未取得、と一次データで確定。フェーズゲート indexed≥50 は未達(20/50)。本タスクは worker/サイトコード不変更の計測ツールのため本番デプロイ面なし（`deploy:prod` 非該当）、本番リリースゲート PASS=29/0/1 で prod 健全確認。**#2 残**: sitemap 再送信は書込スコープ未付与で自律不可→GSC UI 手動。**次推奨=#3 内部リンク強化**（477 URL unknown 解消に直撃、本日の一次データが裏付け）。詳細 docs/ops/daily/2026-06-16.md。

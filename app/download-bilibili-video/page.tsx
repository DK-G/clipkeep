...
import { bilibiliText, normalizeLocale } from '@/lib/i18n/ui';
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  const t = bilibiliText[locale];
  const base = 'https://clipkeep.net';
  const path = '/download-bilibili-video';
...
export default async function Page({ searchParams }: Props) {
  ...
  const websiteUrl = `https://clipkeep.net/download-bilibili-video?locale=${locale}`;
    ...
    '@graph': [
      {
        ...
        'featureList': 'Fast Bilibili video extraction, High-quality MP4',
        'description': t.subtitle
      },
      {
        '@type': 'HowTo',
  ...
  return (
    <>
      ...
      <Suspense fallback={<div className="p-12 text-center text-slate-600 dark:text-slate-400">{loadingText(locale)}</div>}>
        <SnsDownloaderClient locale={locale} platform="bilibili" dict={t} />
      </Suspense>
    </>
  );
}

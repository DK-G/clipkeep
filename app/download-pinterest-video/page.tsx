...
import { pinterestText, normalizeLocale, menuText } from '@/lib/i18n/ui';
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  const t = pinterestText[locale];
  const base = 'https://clipkeep.net';
  const path = '/download-pinterest-video';
...
export default async function Page({ searchParams }: Props) {
  ...
  const websiteUrl = `https://clipkeep.net/download-pinterest-video?locale=${locale}`;
    ...
    '@graph': [
      {
        ...
        'featureList': 'Fast Pinterest video extraction, High-quality MP4',
        'description': t.subtitle,
        'aggregateRating': {
      ...
      },
      {
        ...
        'itemListElement': [
          ...
          },
          {
            ...
            'position': 2,
            'name': t.title,
            'item': `https://clipkeep.net/download-pinterest-video?locale=${locale}`
          }
        ]
      }
    ]
  ...
  return (
    <>
      ...
      <Suspense fallback={<div className="p-12 text-center text-slate-600 dark:text-slate-400">{loadingText(locale)}</div>}>
        <SnsDownloaderClient locale={locale} platform="pinterest" dict={t} />
      </Suspense>
    </>
  );
}

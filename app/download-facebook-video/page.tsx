...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  const base = SITE_URL;
  const path = '/download-facebook-video';
...
export default async function Page({ searchParams }: Props) {
  ...
  const websiteUrl = `${SITE_URL}/download-facebook-video?locale=${locale}`;
      ...
      },
      {
        ...
        'itemListElement': [
          ...
          },
          {
            ...
            'position': 2,
            'name': t.title,
            'item': `${SITE_URL}/download-facebook-video?locale=${locale}`
          }
        ]
      }
    ]
  ...
  return (
    <>
      ...
      <Suspense fallback={<div className="p-12 text-center text-slate-600 dark:text-slate-400">{loadingText(locale)}</div>}>
        <SnsDownloaderClient locale={locale} platform="facebook" dict={t} />
      </Suspense>
    </>
  );
}

...
import { TelegramDownloaderClient } from '@/components/downloaders/telegram-downloader-client';
import { telegramText, normalizeLocale } from '@/lib/i18n/ui';
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
  ...
  const path = '/download-telegram-video';
...
export default async function TelegramDownloaderPage({ searchParams }: Props) {
  const sp = await searchParams;
  const locale = normalizeLocale(typeof sp.locale === 'string' ? sp.locale : undefined);
  const t = telegramText[locale];

  const websiteUrl = `https://clipkeep.net/download-telegram-video?locale=${locale}`;
    ...
    '@graph': [
      {
        ...
        'featureList': 'Secure Telegram video extraction, Channel video downloader, Multilingual support',
        'description': t.subtitle
      },
      {
        '@type': 'HowTo',
  ...
  return (
    <>
      ...
      <Suspense fallback={<div className="p-12 text-center text-slate-600 dark:text-slate-400">{loadingText(locale)}</div>}>
        <TelegramDownloaderClient locale={locale} />
      </Suspense>
    </>
  );
}

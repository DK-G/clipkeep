...
function telegramSpecificFailLines(locale: BlogLocale, slug: string): string[] | null {
  ...
  const ja: Record<string, string[]> = {
    'telegram-video-download': ['投稿リンクが非公開チャット由来でした。', 'チャンネル内でそのメディアに制限がかかっています。', 'Telegram CDN への取得がタイムアウトしました。'],
    'telegram-video-downloader-online': ['URL ではなく共有文面をコピーしていました。', 'アプリ設定でリンクプレビューが無効になっています。', 'ブラウザ拡張がリクエストヘッダーと競合しています。'],
    'telegram-video-save-method-ja': ['元リンクではなく転送メッセージのリンクを使っていました。', '元の投稿が編集されてメディアが差し替わりました。', '地域側のエッジが古いメタデータを返しました。'],
    'telegram-video-download-pc-ja': ['デスクトップアプリが内部 deep-link をコピーしていました。', '社内ネットワークが Telegram のメディアドメインをブロックしています。', 'アンチウイルスのプロキシ検査でダウンロードが止まりました。'],
    'telegram-video-download-iphone': ['iOS 側で短いリダイレクト URL だけがコピーされました。', 'アプリ内ブラウザが必要な query parameter を削除しています。', 'バックグラウンド更新でリクエストが停止しました。'],
    'telegram-video-download-android': ['Android WebView が referrer を落としました。', '省電力設定でバックグラウンド抽出が一時停止しました。', '共有 intent が壊れた URL エンコードを渡しています。'],
    'telegram-private-video-download': ['対象メディアは公開状態ではありません。', 'チャンネル参加に承認が必要です。', '非公開投稿への権限が期限切れになりました。'],
    'telegram-video-download-link': ['リンク先がメディアなしのメッセージでした。', 'メディア付き投稿の URL ではなくスレッドの先頭 URL をコピーしていました。', 'URL に未対応のトラッキング断片が含まれています。'],
    'telegram-save-visible-ja': ['ダウンロード履歴と通知を混同しています。', '公開チャンネルの管理者が見えるのは view 数だけです。', '共有ファイルによってアプリ内でユーザー名が見えることがあります。'],
    'telegram-video-download-bot-safe': ['ボットが過剰な権限を求めています。', '未検証のボットでポリシーページがありません。', 'ボットが不明な外部ストレージ経由でメディアを中継しています。'],
  };
  ...
  const es: Record<string, string[]> = {
    ...
    'telegram-video-download-link': ['El enlace apunta a un mensaje sin vídeo.', 'Se copió la raíz del hilo y no el post.', 'La URL trae fragmentos de tracking no compatibles.'],
  ...
  const fr: Record<string, string[]> = {
    'telegram-video-download': ['Le lien de la publication provient d’un chat privé.', 'Le média est restreint à l’intérieur du canal.', 'La requête vers le CDN de Telegram a expiré.'],
    'telegram-video-downloader-online': ['Le texte de partage a été copié au lieu de l’URL.', 'L’aperçu du lien est bloqué dans les réglages de l’application.', 'Une extension du navigateur est entrée en conflit avec les en-têtes de la requête.'],
    'telegram-video-save-method-ja': ['Un lien de transfert a été utilisé à la place du lien d’origine.', 'La publication d’origine a été modifiée et le média a changé.', 'Le nœud régional a renvoyé des métadonnées obsolètes.'],
    'telegram-video-download-pc-ja': ['L’application de bureau a copié un deep-link interne.', 'Le réseau d’entreprise bloque les domaines média de Telegram.', 'Le téléchargement a été interrompu par une analyse antivirus via proxy.'],
    'telegram-video-download-iphone': ['L’iPhone a copié uniquement un lien court de redirection.', 'Le navigateur intégré a supprimé des paramètres nécessaires.', 'La mise à jour en arrière-plan a suspendu la requête.'],
    'telegram-video-download-android': ['Le WebView Android a supprimé le referrer.', 'Le mode économie a mis l’extraction en pause en arrière-plan.', 'Le partage a fourni une URL avec un encodage incorrect.'],
    'telegram-private-video-download': ['Le média visé n’est pas accessible publiquement.', 'Le canal exige une approbation pour y entrer.', 'L’autorisation de la publication privée a expiré.'],
    'telegram-video-download-link': ['Le lien pointe vers un message sans média.', 'La racine du fil a été copiée au lieu de l’URL de la publication contenant le média.', 'L’URL contient un fragment de suivi non pris en charge.'],
    ...
    'telegram-video-download-bot-safe': ['Le bot demande des autorisations excessives.', 'Le service du bot non vérifié ne propose pas de page de politique.', 'Le bot relaie le média via un stockage externe inconnu.'],
  };
  const de: Record<string, string[]> = {
    'telegram-video-download': ['Der Beitragslink stammt aus einem privaten Chat.', 'Das Medium ist innerhalb des Kanals eingeschränkt.', 'Die Anfrage an das Telegram-CDN ist abgelaufen.'],
    'telegram-video-downloader-online': ['Es wurde der Freigabetext statt der URL kopiert.', 'Die Linkvorschau ist in den App-Einstellungen blockiert.', 'Eine Browser-Erweiterung hat die Anfrage-Header gestört.'],
    'telegram-video-save-method-ja': ['Es wurde ein Weiterleitungslink statt des Original-Links verwendet.', 'Der Originalbeitrag wurde bearbeitet und das Medium hat sich geändert.', 'Der regionale Knoten hat veraltete Metadaten zurückgegeben.'],
    'telegram-video-download-pc-ja': ['Die Desktop-App hat einen internen Deep-Link kopiert.', 'Das Unternehmensnetz blockiert Telegram-Media-Domains.', 'Der Download wurde durch einen Antivirus-Scan über einen Proxy unterbrochen.'],
    'telegram-video-download-iphone': ['Das iPhone hat nur einen kurzen Weiterleitungslink kopiert.', 'Der In-App-Browser hat notwendige Parameter entfernt.', 'Die Hintergrundaktualisierung hat die Anfrage unterbrochen.'],
    'telegram-video-download-android': ['Die Android-WebView hat den Referrer entfernt.', 'Der Energiesparmodus hat die Extraktion im Hintergrund pausiert.', 'Die Freigabe hat eine URL mit fehlerhafter Kodierung geliefert.'],
    'telegram-private-video-download': ['Das Zielmedium ist nicht öffentlich zugänglich.', 'Der Kanal erfordert eine Freigabe für den Beitritt.', 'Die Berechtigung für den privaten Beitrag ist abgelaufen.'],
    'telegram-video-download-link': ['Der Link verweist auf eine Nachricht ohne Medium.', 'Es wurde der Thread-Anfang statt der URL des Beitrags mit Medium kopiert.', 'Die URL enthält ein nicht unterstütztes Tracking-Fragment.'],
    ...
    'telegram-video-download-bot-safe': ['Der Bot fordert zu weitgehende Berechtigungen an.', 'Der nicht verifizierte Bot-Dienst bietet keine Richtlinienseite an.', 'Der Bot leitet das Medium über einen unbekannten externen Speicher weiter.'],
  };
  ...
  const tr: Record<string, string[]> = {
    'telegram-video-download': ['Gönderi bağlantısı özel bir sohbetten alınmış.', 'Medya kanal içinde kısıtlanmış olabilir.', 'Telegram CDN isteği zaman aşımına uğradı.'],
    'telegram-video-downloader-online': ['URL yerine paylaşım metni kopyalandı.', 'Bağlantı önizlemesi uygulama ayarları tarafından engelleniyor.', 'Bir tarayıcı eklentisi istek başlıklarıyla çakıştı.'],
    'telegram-video-save-method-ja': ['Kaynak bağlantı yerine yönlendirilmiş mesaj bağlantısı kullanıldı.', 'Orijinal gönderi düzenlendi ve medya değişti.', 'Bölgesel uç düğüm eski meta veriler döndürdü.'],
    'telegram-video-download-pc-ja': ['Masaüstü uygulaması dahili bir derin bağlantı kopyaladı.', 'Kurumsal ağ Telegram medya alanlarını engelliyor.', 'İndirme antivirüs proxy taraması yüzünden kesildi.'],
    'telegram-video-download-iphone': ['iOS yalnızca kısa yönlendirme bağlantısını kopyaladı.', 'Uygulama içi tarayıcı gerekli sorgu parametrelerini kaldırdı.', 'Arka plan yenilemesi isteği askıya aldı.'],
    'telegram-video-download-android': ['Android WebView yönlendiren kaynağı kaldırdı.', 'Pil tasarrufu arka plandaki çıkarma işlemini duraklattı.', 'Paylaşım intenti bozuk URL kodlaması üretti.'],
    'telegram-private-video-download': ['Hedef medya herkese açık değil.', 'Kanal katılım onayı gerektiriyor.', 'Özel gönderi izni süresi doldu.'],
    'telegram-video-download-link': ['Bağlantı medyasız bir mesaja gidiyor.', 'Medya gönderisinin URL’si yerine dizinin kök bağlantısı kopyalandı.', 'URL desteklenmeyen bir takip parçası içeriyor.'],
    ...
    'telegram-video-download-bot-safe': ['Bot aşırı izinler istiyor.', 'Doğrulanmamış bot uç noktasının bir politika sayfası yok.', 'Bot medyayı bilinmeyen bir üçüncü taraf depolama üzerinden iletiyor.'],
  };

  const id: Record<string, string[]> = {
    'telegram-video-download': ['Tautan postingan berasal dari chat privat.', 'Media dibatasi di dalam kanal tersebut.', 'Permintaan ke CDN Telegram mengalami timeout.'],
    'telegram-video-downloader-online': ['Yang disalin adalah teks bagikan, bukan URL.', 'Pratinjau tautan diblokir oleh pengaturan aplikasi.', 'Ekstensi browser bentrok dengan header permintaan.'],
    'telegram-video-save-method-ja': ['Yang digunakan adalah tautan forward pesan, bukan tautan sumber.', 'Postingan asli telah diedit dan medianya berubah.', 'Node edge regional mengembalikan metadata yang sudah usang.'],
    'telegram-video-download-pc-ja': ['Aplikasi desktop menyalin deep-link internal.', 'Jaringan kantor memblokir domain media Telegram.', 'Unduhan terganggu oleh pemindaian antivirus melalui proxy.'],
    'telegram-video-download-iphone': ['iOS hanya menyalin URL pengalihan pendek.', 'Browser di dalam aplikasi menghapus query parameter penting.', 'Penyegaran latar belakang menangguhkan permintaan.'],
    'telegram-video-download-android': ['WebView Android menghapus referrer.', 'Mode hemat baterai menjeda ekstraksi di latar belakang.', 'Intent berbagi menghasilkan encoding URL yang rusak.'],
    'telegram-private-video-download': ['Media target tidak bisa diakses secara publik.', 'Kanal membutuhkan persetujuan keanggotaan.', 'Izin postingan privat sudah kedaluwarsa.'],
    'telegram-video-download-link': ['Tautan mengarah ke pesan tanpa media.', 'Yang disalin adalah URL akar thread, bukan URL postingan medianya.', 'URL mengandung fragmen pelacakan yang tidak didukung.'],
    ...
    'telegram-video-download-bot-safe': ['Bot meminta izin yang berlebihan.', 'Endpoint bot yang belum terverifikasi tidak memiliki halaman kebijakan.', 'Bot meneruskan media melalui penyimpanan pihak ketiga yang tidak dikenal.'],
  };

  const hi: Record<string, string[]> = {
    'telegram-video-download': ['पोस्ट का लिंक किसी private chat से आया है।', 'Media चैनल के भीतर restricted है।', 'Telegram CDN request timeout हो गई।'],
    'telegram-video-downloader-online': ['URL की जगह share text कॉपी हो गया।', 'App settings की वजह से link preview block है।', 'Browser extension request headers से टकरा रही है।'],
    'telegram-video-save-method-ja': ['Source link की जगह forwarded message link इस्तेमाल हुई।', 'Original post edit हुई और media बदल गया।', 'Regional edge node ने stale metadata लौटाया।'],
    'telegram-video-download-pc-ja': ['Desktop app ने internal deep-link कॉपी किया।', 'Corporate network Telegram media domains block कर रहा है।', 'Antivirus proxy scan ने download रोक दिया।'],
    'telegram-video-download-iphone': ['iOS ने सिर्फ short redirect URL कॉपी की।', 'In-app browser ने ज़रूरी query params हटा दिए।', 'Background refresh ने request suspend कर दी।'],
    'telegram-video-download-android': ['Android WebView ने referrer हटा दिया।', 'Battery saver ने background extraction pause कर दी।', 'Share intent ने malformed URL encoding दी।'],
    'telegram-private-video-download': ['Target media publicly accessible नहीं है।', 'Channel में membership approval की ज़रूरत है।', 'Private post permission expire हो गई।'],
    'telegram-video-download-link': ['Link ऐसे message पर जा रहा है जिसमें media नहीं है।', 'Media post URL की जगह thread root URL कॉपी हुई।', 'URL में unsupported tracking fragment है।'],
    ...
    'telegram-video-download-bot-safe': ['Bot बहुत ज़्यादा permissions मांग रहा है।', 'Unverified bot endpoint की कोई policy page नहीं है।', 'Bot media को किसी unknown third-party storage से forward कर रहा है।'],
  };

  const pt: Record<string, string[]> = {
    'telegram-video-download': ['O link da publicação veio de um chat privado.', 'A mídia está restrita dentro do canal.', 'A requisição ao CDN do Telegram expirou.'],
    'telegram-video-downloader-online': ['Foi copiado o texto de compartilhamento, não a URL.', 'A visualização do link está bloqueada nas configurações do app.', 'Uma extensão do navegador entrou em conflito com os cabeçalhos da requisição.'],
    'telegram-video-save-method-ja': ['Foi usado um link de encaminhamento em vez do link original.', 'A publicação original foi editada e a mídia mudou.', 'O nó regional retornou metadados desatualizados.'],
    'telegram-video-download-pc-ja': ['O app de desktop copiou um deep-link interno.', 'A rede corporativa bloqueia domínios de mídia do Telegram.', 'O download foi interrompido por uma verificação de antivírus em proxy.'],
    'telegram-video-download-iphone': ['O iPhone copiou apenas um link curto de redirecionamento.', 'O navegador interno removeu parâmetros necessários.', 'A atualização em segundo plano suspendeu a requisição.'],
    'telegram-video-download-android': ['O WebView do Android removeu o referrer.', 'O modo de economia pausou a extração em segundo plano.', 'O compartilhamento entregou uma URL com codificação incorreta.'],
    'telegram-private-video-download': ['A mídia alvo não está acessível publicamente.', 'O canal exige aprovação para entrada.', 'A permissão da publicação privada expirou.'],
    'telegram-video-download-link': ['O link aponta para uma mensagem sem mídia.', 'Foi copiada a raiz do tópico, não a URL da publicação com a mídia.', 'A URL contém um fragmento de rastreamento não suportado.'],
    ...
    'telegram-video-download-bot-safe': ['O bot pede permissões excessivas.', 'O endpoint do bot não verificado não oferece página de política.', 'O bot encaminha a mídia por um armazenamento externo desconhecido.'],
  };
...
function twitterSpecificFailLines(locale: BlogLocale, slug: string): string[] | null {
  ...
  const ja: Record<string, string[]> = {
    'how-to-download-twitter-videos': ['削除済みの投稿から URL をコピーしていました。', '動画は引用ポスト側にあり、別のリンクを使っていました。', '短縮された t.co リンクを正しく展開できていません。'],
    'twitter-video-downloader-online-free': ['短時間にリクエストが集中して一時的な制限に当たりました。', '広告ブロッカーがメディア取得の流れを止めています。', '投稿 URL ではなくプロフィール URL を貼り付けていました。'],
    'twitter-video-download-iphone': ['iPhone のアプリ内ブラウザが必要なパラメータを落としています。', '共有メニューでプレビュー用 URL しかコピーされていません。', '省電力モードでバックグラウンド処理が止まりました。'],
    'twitter-video-save-method-ja': ['動画を含まない投稿 URL をコピーしていました。', 'その投稿のメディアは地域制限付きです。', 'X 側の一時レスポンスでメタデータが欠けていました。'],
    'twitter-video-save-not-working-ja': ['URL をコピーした後に投稿が非公開になりました。', 'アカウントが保護状態または停止状態に変わりました。', 'セッション依存のメディア取得が失敗しました。'],
    'twitter-video-download-android': ['Android WebView が URL エンコードを変えてしまいました。', 'バッテリー最適化で抽出処理が一時停止しました。', '取得中に Wi-Fi とモバイル通信が切り替わりました。'],
    'twitter-gif-download': ['GIF は実際には MP4 として配信されています。', '選択した画質ではアニメーション版が用意されていません。', 'GIF と見えても実際は静止画像の投稿です。'],
    'twitter-video-downloader-safe': ['クローンや偽装の疑いがあるドメインを開いていました。', '安全ではないミラー先として mixed content 警告が出ています。', '信頼できない拡張機能がリダイレクトを差し込んでいます。'],
    'twitter-video-download-no-watermark': ['元の動画にすでに投稿者側の透かしがあります。', '誤って低画質の代替版を選んでいました。', '期待していた形式のメディアバリエーションがありません。'],
    'twitter-video-download-mp4': ['その投稿では HLS 版しか公開されていません。', '一時的な CDN エラーで MP4 トラックが取れませんでした。', '選んだ画質には直接 MP4 の URL がありません。'],
    'twitter-video-download-chrome': ['Chrome の古いキャッシュが古いスクリプトを返しています。', 'サードパーティ Cookie の設定でメディアトークン取得が壊れています。', '拡張機能が fetch 処理と競合しています。'],
    'twitter-save-visible-ja': ['表示回数とダウンロード履歴を混同しています。', '見えるのは公開エンゲージメントで、端末保存そのものではありません。', '再投稿や共有によってアカウントの動きが見えることがあります。'],
    'twitter-video-download-private-account': ['保護アカウントのメディアは公開取得できません。', 'メディアを見るにはフォロー承認が必要です。', 'URL コピー後に投稿の権限設定が変わりました。'],
    'twitter-video-download-without-login': ['ゲスト用トークンの有効期限が切れています。', 'その投稿はログイン済みセッションを前提にしています。', '年齢制限や国制限で未ログインアクセスが遮断されています。'],
    'twitter-video-downloader-2026': ['プラットフォーム側のエンドポイントが変わり、キャッシュが古くなっています。', '新しい anti-abuse 制限に引っかかっています。', '旧来の抽出ルートがもう使えません。'],
  };

  const es: Record<string, string[]> = {
    'how-to-download-twitter-videos': ['La URL proviene de un tweet eliminado.', 'El vídeo está en un quote tweet y se copió otro enlace.', 'El enlace t.co no se resolvió.'],
    ...
    'twitter-video-save-method-ja': ['Se copió un tweet sin vídeo.', 'El vídeo tiene restricción regional.', 'El nodo de X devolvió metadatos incompletos.'],
    ...
    'twitter-video-download-no-watermark': ['La marca de água ya existe en origen.', 'Se eligió por error una calidad baja.', 'Falta variante de formato en ese tweet.'],
    ...
    'twitter-video-download-private-account': ['El contenido de cuenta protegida no es público.', 'Se requiere aprobación para acceder al vídeo.', 'El permiso cambió tras copiar la URL.'],
  ...
  const fr: Record<string, string[]> = {
    'how-to-download-twitter-videos': ['L’URL a été copiée depuis une publication déjà supprimée.', 'La vidéo se trouvait dans une publication citée et le lien utilisé n’était pas le bon.', 'Le lien court t.co n’a pas été résolu correctement.'],
    'twitter-video-downloader-online-free': ['La limite temporaire de requêtes a été atteinte.', 'Le bloqueur a interrompu la chaîne de chargement du média.', 'L’URL du profil a été collée à la place de l’URL de la publication.'],
    'twitter-video-download-iphone': ['Le navigateur intégré de l’iPhone a supprimé des paramètres importants.', 'Le menu de partage a copié uniquement l’URL d’aperçu.', 'Le mode économie d’énergie a suspendu le téléchargement en arrière-plan.'],
    'twitter-video-save-method-ja': ['Une publication sans vidéo a été copiée.', 'Le média de la publication est restreint par région.', 'Le nœud X a renvoyé des métadonnées incomplètes.'],
    'twitter-video-save-not-working-ja': ['La publication est devenue privée après la copie du lien.', 'Le compte a été protégé ou suspendu.', 'La requête dépendante de la session a échoué.'],
    'twitter-video-download-android': ['Le WebView Android a modifié l’encodage de l’URL.', 'L’optimisation de la batterie a mis l’extraction en pause.', 'Le réseau a basculé entre Wi-Fi et données mobiles pendant la requête.'],
    ...
    'twitter-video-downloader-safe': ['Un domaine cloné ou suspect a été ouvert.', 'Le navigateur a signalé un contenu non sûr.', 'Une extension non fiable a injecté une redirection.'],
    'twitter-video-download-no-watermark': ['Le fichier source contient déjà un filigrane.', 'Une variante de faible qualité a été choisie par erreur.', 'La variante de format attendue n’est pas apparue pour cette publication.'],
    'twitter-video-download-mp4': ['Cette publication n’expose que la variante HLS.', 'Une erreur CDN temporaire a bloqué la piste MP4.', 'La qualité choisie ne fournit pas d’URL MP4 directe.'],
    'twitter-video-download-chrome': ['Un ancien cache de Chrome a servi des scripts obsolètes.', 'La configuration des cookies a empêché le jeton du média.', 'Une extension est entrée en conflit avec le chargement de la page.'],
    ...
    'twitter-video-download-private-account': ['Le média d’un compte protégé n’est pas accessible publiquement.', 'Il faut être approuvé pour suivre le compte avant d’accéder au contenu.', 'L’autorisation de la publication a changé après la copie du lien.'],
    'twitter-video-download-without-login': ['Le jeton d’accès visiteur a expiré.', 'La publication exige une session authentifiée.', 'Une restriction d’âge ou de pays a bloqué l’accès sans connexion.'],
    'twitter-video-downloader-2026': ['Le point d’accès de la plateforme a changé et le cache est devenu obsolète.', 'De nouveaux seuils anti-abus ont été déclenchés.', 'L’ancienne route d’extraction ne fonctionne plus.'],
  };
  const de: Record<string, string[]> = {
    'how-to-download-twitter-videos': ['Die URL wurde aus einem bereits gelöschten Beitrag kopiert.', 'Das Video befand sich in einem zitierten Beitrag und es wurde der falsche Link verwendet.', 'Der verkürzte t.co-Link konnte nicht korrekt aufgelöst werden.'],
    'twitter-video-downloader-online-free': ['Das temporäre Anfragelimit wurde erreicht.', 'Ein Blocker hat die Lade-Kette des Mediums unterbrochen.', 'Es wurde die Profil-URL statt der Beitrags-URL eingefügt.'],
    'twitter-video-download-iphone': ['Der In-App-Browser auf dem iPhone hat wichtige Parameter entfernt.', 'Über das Teilen-Menü wurde nur die Vorschau-URL kopiert.', 'Der Stromsparmodus hat den Download im Hintergrund angehalten.'],
    'twitter-video-save-method-ja': ['Es wurde ein Beitrag ohne Video kopiert.', 'Das Medium des Beitrags ist regional eingeschränkt.', 'Ein X-Endpunkt hat unvollständige Metadaten zurückgegeben.'],
    'twitter-video-save-not-working-ja': ['Der Beitrag wurde nach dem Kopieren des Links privat.', 'Das Konto wurde geschützt oder gesperrt.', 'Die sitzungsabhängige Anfrage ist fehlgeschlagen.'],
    'twitter-video-download-android': ['Die Android-WebView hat die URL-Kodierung verändert.', 'Die Akkuoptimierung hat die Extraktion pausiert.', 'Das Netzwerk wechselte während der Anfrage zwischen WLAN und mobilen Daten.'],
    ...
    'twitter-video-downloader-safe': ['Es wurde eine geklonte oder verdächtige Domain geöffnet.', 'Der Browser hat vor unsicherem Inhalt gewarnt.', 'Eine nicht vertrauenswürdige Erweiterung hat eine Weiterleitung eingefügt.'],
    'twitter-video-download-no-watermark': ['Die Quelldatei enthält bereits ein Wasserzeichen.', 'Aus Versehen wurde eine Variante mit niedriger Qualität gewählt.', 'Die erwartete Formatvariante ist für diesen Beitrag nicht verfügbar.'],
    'twitter-video-download-mp4': ['Dieser Beitrag stellt nur die HLS-Variante bereit.', 'Ein temporärer CDN-Fehler hat die MP4-Spur blockiert.', 'Die gewählte Qualität liefert keine direkte MP4-URL.'],
    'twitter-video-download-chrome': ['Ein alter Chrome-Cache hat veraltete Skripte ausgeliefert.', 'Die Cookie-Einstellung hat das Medien-Token blockiert.', 'Eine Erweiterung hat den Seitenaufbau gestört.'],
    ...
    'twitter-video-download-private-account': ['Medien eines geschützten Kontos sind nicht öffentlich zugänglich.', 'Vor dem Zugriff auf den Inhalt ist eine Bestätigung zum Folgen erforderlich.', 'Die Berechtigung des Beitrags hat sich nach dem Kopieren des Links geändert.'],
    'twitter-video-download-without-login': ['Das Besucher-Token ist abgelaufen.', 'Der Beitrag verlangt eine authentifizierte Sitzung.', 'Eine Alters- oder Ländersperre blockiert den Zugriff ohne Anmeldung.'],
    'twitter-video-downloader-2026': ['Der Plattform-Endpunkt wurde geändert und der Cache ist veraltet.', 'Neue Anti-Missbrauchs-Schwellen wurden ausgelöst.', 'Der alte Extraktionspfad wird nicht mehr unterstützt.'],
  };

  const id: Record<string, string[]> = {
    'how-to-download-twitter-videos': ['URL tweet diambil dari postingan yang sudah dihapus.', 'Medianya ada di tweet kutipan tetapi tautan yang dipakai bukan tautan yang benar.', 'Tautan pendek t.co gagal diarahkan ke tujuan akhir.'],
    'twitter-video-downloader-online-free': ['Batas permintaan sementara tercapai.', 'Pemblokir iklan memutus rantai pemuatan media.', 'Yang ditempel adalah URL profil, bukan URL tweet.'],
    'twitter-video-download-iphone': ['Browser di dalam aplikasi iPhone menghapus parameter penting.', 'Menu berbagi hanya menyalin URL pratinjau.', 'Mode hemat daya menghentikan unduhan di latar belakang.'],
    'twitter-video-save-method-ja': ['Yang disalin adalah tweet tanpa video.', 'Media pada tweet dibatasi berdasarkan wilayah.', 'Respons edge X sementara mengembalikan metadata yang tidak lengkap.'],
    'twitter-video-save-not-working-ja': ['Tweet menjadi privat setelah tautannya disalin.', 'Akun berubah menjadi terlindungi atau ditangguhkan.', 'Permintaan media yang bergantung pada sesi gagal.'],
    'twitter-video-download-android': ['WebView Android mengubah encoding URL.', 'Optimasi baterai menjeda proses ekstraksi.', 'Jaringan berpindah antara Wi-Fi dan data seluler saat permintaan berjalan.'],
    ...
    'twitter-video-downloader-safe': ['Pengguna membuka domain tiruan yang mencurigakan.', 'Browser menampilkan peringatan mixed content pada mirror yang tidak aman.', 'Ekstensi yang tidak tepercaya menyisipkan skrip pengalihan.'],
    'twitter-video-download-no-watermark': ['File sumber sudah memiliki watermark pembuat.', 'Varian kualitas rendah dipilih tanpa sengaja.', 'Varian format yang diharapkan tidak tersedia pada tweet ini.'],
    'twitter-video-download-mp4': ['Tweet ini hanya menyediakan varian HLS.', 'Track MP4 terblokir oleh error CDN sementara.', 'Kualitas yang dipilih tidak memiliki URL MP4 langsung.'],
    'twitter-video-download-chrome': ['Cache Chrome yang lama masih memuat bundle skrip usang.', 'Pengaturan cookie pihak ketiga merusak pengambilan token media.', 'Ekstensi browser bentrok dengan proses fetch.'],
    ...
    'twitter-video-download-private-account': ['Media dari akun terlindungi tidak tersedia untuk publik.', 'Akses ke media memerlukan persetujuan follow terlebih dahulu.', 'Izin pada tweet berubah setelah URL disalin.'],
    'twitter-video-download-without-login': ['Token akses tamu sudah kedaluwarsa.', 'Postingan memerlukan sesi yang sudah terautentikasi.', 'Pembatasan usia atau negara memblokir akses tanpa login.'],
    'twitter-video-downloader-2026': ['Endpoint platform berubah dan cache sudah usang.', 'Ambang anti-abuse yang baru terpicu.', 'Jalur ekstraksi lama sudah tidak lagi didukung.'],
  };

  const hi: Record<string, string[]> = {
    'how-to-download-twitter-videos': ['Tweet की URL किसी डिलीट हो चुकी पोस्ट से कॉपी की गई थी।', 'वीडियो quote tweet में था, लेकिन गलत लिंक इस्तेमाल किया गया।', 'छोटी t.co लिंक अंतिम पते तक resolve नहीं हुई।'],
    'twitter-video-downloader-online-free': ['बार-बार request करने से अस्थायी rate limit लग गई।', 'Ad-blocker ने media request chain को रोक दिया।', 'Tweet URL की जगह profile URL पेस्ट कर दी गई।'],
    'twitter-video-download-iphone': ['iPhone के in-app browser ने ज़रूरी parameters हटा दिए।', 'Share sheet ने सिर्फ preview URL कॉपी की।', 'Low-power mode ने background download रोक दिया।'],
    'twitter-video-save-method-ja': ['यूज़र ने ऐसी tweet URL कॉपी की जिसमें वीडियो नहीं था।', 'Tweet का media किसी region में restricted है।', 'X edge से अधूरा metadata लौटा।'],
    'twitter-video-save-not-working-ja': ['लिंक कॉपी करने के बाद tweet private हो गया।', 'अकाउंट protected या suspended हो गया।', 'Session पर निर्भर media request फेल हो गई।'],
    'twitter-video-download-android': ['Android WebView ने URL encoding बदल दी।', 'Battery optimization ने extraction रोक दी।', 'Request के दौरान network Wi-Fi और mobile data के बीच बदल गया।'],
    ...
    'twitter-video-downloader-safe': ['यूज़र ने cloned phishing domain खोल लिया।', 'Browser ने unsafe mirror पर mixed-content warning दिखाई।', 'Untrusted extension ने redirect script inject किया।'],
    'twitter-video-download-no-watermark': ['Source file में creator watermark पहले से मौजूद है।', 'गलती से low-quality fallback चुन लिया गया।', 'चुने हुए format में tweet media variant उपलब्ध नहीं है।'],
    'twitter-video-download-mp4': ['उस post के लिए सिर्फ HLS variant उपलब्ध है।', 'अस्थायी CDN error ने MP4 track block कर दिया।', 'चुनी हुई quality में direct MP4 URL नहीं है।'],
    'twitter-video-download-chrome': ['पुराना Chrome cache outdated script bundle दे रहा है।', 'Third-party cookie setting ने media token fetch तोड़ दिया।', 'एक extension fetch process से टकरा रही है।'],
    ...
    'twitter-video-download-private-account': ['Protected account का media publicly accessible नहीं है।', 'Media access से पहले follow approval ज़रूरी है।', 'URL कॉपी होने के बाद tweet permission बदल गई।'],
    'twitter-video-download-without-login': ['Guest access token expire हो गया।', 'Post को authenticated session की ज़रूरत है।', 'Age या country gate ने unauthenticated access रोक दिया।'],
    'twitter-video-downloader-2026': ['Platform endpoint बदल गया और cache पुराना हो चुका है।', 'नए anti-abuse thresholds trigger हो गए।', 'पुराना extraction path अब supported नहीं है।'],
  };

  const pt: Record<string, string[]> = {
    'how-to-download-twitter-videos': ['A URL foi copiada de uma publicação já apagada.', 'O vídeo estava em uma publicação citada e o link usado não era o correto.', 'O link curto t.co não foi resolvido corretamente.'],
    'twitter-video-downloader-online-free': ['O limite temporário de requisições foi atingido.', 'O bloqueador interrompeu a cadeia de carregamento da mídia.', 'Foi colada a URL do perfil em vez da URL da publicação.'],
    'twitter-video-download-iphone': ['O navegador interno do iPhone removeu parâmetros importantes.', 'O menu de compartilhamento copiou apenas a URL de visualização.', 'O modo de economia pausou o download em segundo plano.'],
    'twitter-video-save-method-ja': ['Foi copiada uma publicação sem vídeo.', 'A mídia da publicação está restrita por região.', 'A borda do X retornou metadados incompletos.'],
    'twitter-video-save-not-working-ja': ['A publicação ficou privada depois da cópia do link.', 'A conta foi protegida ou suspensa.', 'A requisição dependente de sessão falhou.'],
    'twitter-video-download-android': ['O WebView do Android alterou a codificação da URL.', 'A otimização de bateria pausou a extração.', 'A rede mudou entre Wi-Fi e dados durante a requisição.'],
    ...
    'twitter-video-downloader-safe': ['Foi aberto um domínio clonado ou suspeito.', 'O navegador avisou sobre conteúdo inseguro.', 'Uma extensão não confiável injetou um redirecionamento.'],
    'twitter-video-download-no-watermark': ['A marca d’água já existe no arquivo de origem.', 'Uma variante de baixa qualidade foi escolhida por engano.', 'A variante de formato esperada não apareceu nessa publicação.'],
    'twitter-video-download-mp4': ['Essa publicação só expõe a variante HLS.', 'Um erro temporário de CDN bloqueou a trilha MP4.', 'A qualidade escolhida não tem URL MP4 direta.'],
    'twitter-video-download-chrome': ['O cache antigo do Chrome serviu scripts desatualizados.', 'A configuração de cookies impediu o token da mídia.', 'Uma extensão entrou em conflito com o carregamento da página.'],
    ...
    'twitter-video-download-private-account': ['A mídia de uma conta protegida não está disponível publicamente.', 'É preciso ter aprovação para seguir a conta antes de acessar o conteúdo.', 'A permissão da publicação mudou depois da cópia do link.'],
    'twitter-video-download-without-login': ['O token de acesso de visitante expirou.', 'A publicação exige uma sessão autenticada.', 'Uma restrição de idade ou país bloqueou o acesso sem login.'],
    'twitter-video-downloader-2026': ['O endpoint da plataforma mudou e o cache ficou defasado.', 'Novos limites antiabuso foram acionados.', 'A rota antiga de extração deixou de funcionar.'],
  };

  const tr: Record<string, string[]> = {
    'how-to-download-twitter-videos': ['URL, daha önce silinmiş bir gönderiden kopyalandı.', 'Video alıntılanmış bir gönderideydi ve yanlış bağlantı kullanıldı.', 'Kısaltılmış t.co bağlantısı düzgün şekilde çözümlenemedi.'],
    'twitter-video-downloader-online-free': ['Geçici istek sınırına ulaşıldı.', 'Bir engelleyici medya yükleme zincirini kesti.', 'Gönderi URL’si yerine profil URL’si yapıştırıldı.'],
    'twitter-video-download-iphone': ['iPhone iç tarayıcısı gerekli parametreleri kaldırdı.', 'Paylaş menüsü yalnızca önizleme URL’sini kopyaladı.', 'Düşük güç modu arka plandaki indirmeyi durdurdu.'],
    'twitter-video-save-method-ja': ['Video içermeyen bir gönderi kopyalandı.', 'Gönderideki medya bölgesel olarak kısıtlı.', 'Bir X uç noktası eksik meta veri döndürdü.'],
    'twitter-video-save-not-working-ja': ['Bağlantı kopyalandıktan sonra gönderi gizliye alındı.', 'Hesap korumalı hâle geldi ya da askıya alındı.', 'Oturuma bağlı istek başarısız oldu.'],
    'twitter-video-download-android': ['Android WebView URL kodlamasını değiştirdi.', 'Pil optimizasyonu çıkarma işlemini duraklattı.', 'İstek sırasında ağ Wi‑Fi ile mobil veri arasında değişti.'],
    ...
    'twitter-video-downloader-safe': ['Klonlanmış ya da şüpheli bir alan adı açıldı.', 'Tarayıcı güvensiz içerik uyarısı verdi.', 'Güvenilmeyen bir eklenti yönlendirme enjekte etti.'],
    'twitter-video-download-no-watermark': ['Kaynak dosyada zaten filigran var.', 'Yanlışlıkla düşük kaliteli bir varyant seçildi.', 'Beklenen format varyantı bu gönderi için mevcut değil.'],
    'twitter-video-download-mp4': ['Bu gönderi yalnızca HLS varyantını sunuyor.', 'Geçici bir CDN hatası MP4 parçasını engelledi.', 'Seçilen kalite doğrudan MP4 URL’si vermiyor.'],
    'twitter-video-download-chrome': ['Eski Chrome önbelleği güncelliğini yitirmiş betikler sundu.', 'Çerez ayarı medya belirtecini engelledi.', 'Bir eklenti sayfa yüklenmesini bozdu.'],
    ...
    'twitter-video-download-private-account': ['Korumalı bir hesabın medyası herkese açık değildir.', 'İçeriğe erişmeden önce hesabı takip etmek için onay gerekir.', 'Bağlantı kopyalandıktan sonra gönderinin izni değişti.'],
    'twitter-video-download-without-login': ['Ziyaretçi erişim belirteci süresi doldu.', 'Gönderi kimliği doğrulanmış oturum gerektiriyor.', 'Yaş veya ülke kısıtı giriş olmadan erişimi engelledi.'],
    'twitter-video-downloader-2026': ['Platform uç noktası değişti ve önbellek eskidi.', 'Yeni kötüye kullanım eşikleri tetiklendi.', 'Eski çıkarma yolu artık desteklenmiyor.'],
  };
...
function tiktokSpecificFailLines(locale: BlogLocale, slug: string): string[] | null {
  ...
  const ja: Record<string, string[]> = {
    'tiktok-video-download-no-watermark': ['元の動画にすでに透かしが入っています。', '選んだバリエーションが透かし付きの配信に切り替わりました。', '投稿が制限付き配信に変わっています。'],
    ...
    'tiktok-video-save-method-ja': ['短縮リンクだけをコピーして最終 URL まで解決していません。', '編集された再投稿のあとで動画が差し替えられました。', '選んだ投稿は動画ではなく画像カルーセルです。'],
    'tiktok-video-save-not-working-ja': ['投稿が削除されたか非公開になりました。', 'リンクをコピーした後でアカウントの公開範囲が変わりました。', 'プラットフォーム側の一時制限で抽出が止まっています。'],
    'download-tiktok-video-hd': ['この投稿には HD 版のバリエーションがありません。', '元の動画自体が低解像度でした。', '通信状況に応じて低いビットレートの配信が選ばれました。'],
    'tiktok-download-mp4': ['その投稿は分割ストリームしか公開していません。', 'MP4 へまとめる処理が一時的なタイムアウトで失敗しました。', '選んだ画質には直接 MP4 のバリエーションがありません。'],
    'save-tiktok-video-iphone': ['アプリ内ブラウザが必要なパラメータを削除しています。', 'iPhone 側でプレビュー URL しかコピーされていません。', 'バックグラウンド処理が省電力モードで停止しました。'],
    'tiktok-video-download-android': ['Android の intent で不正な形式の URL が渡されました。', 'WebView がメディアのリダイレクト連鎖を止めています。', 'バッテリー最適化で抽出タスクが一時停止しました。'],
    'tiktok-downloader-safe': ['クローンや不審なミラーサイトを開いていました。', '拡張機能が差し込んだスクリプトで遷移先が変わっています。', '信頼できないドメイン上の TLS 警告を無視していました。'],
    'tiktok-video-downloader-free': ['無料エンドポイントの一時クォータ上限に達しました。', '短時間の連続再試行で anti-abuse のクールダウンに入りました。', '取得中に公開 CDN の経路が切り替わりました。'],
    'tiktok-video-download-private': ['非公開アカウントの投稿は公開取得できません。', 'メディアへアクセスする前にフォロー承認が必要です。', 'コピーから抽出までの間に権限設定が変わりました。'],
    ...
    'tiktok-video-downloader-2026': ['プラットフォーム側のエンドポイントが変わり、キャッシュが古くなっています。', '新しい anti-abuse 閾値で一時的にリクエストが止められています。', '旧来の抽出ルートは廃止されています。'],
    'tiktok-video-link-download': ['リンクが特定の投稿ではなくチャンネルやトップを指しています。', '短縮 URL が最終メディア URL まで展開されていません。', 'トラッキング用の断片がパーサーの正規化を壊しています。'],
  };

  const es: Record<string, string[]> = {
    'tiktok-video-download-no-watermark': ['El vídeo ya trae marca de água en origen.', 'La variante elegida cae a stream con marca.', 'El post cambió a distribución restringida.'],
    ...
    'tiktok-video-save-method-ja': ['Se copió enlace corto sin resolver URL final.', 'El vídeo cambió tras edición del repost.', 'El post es carrusel de imágenes, no vídeo.'],
    ...
    'download-tiktok-video-hd': ['No existe variante HD para ese vídeo.', 'El original era de baja resolución.', 'La red forzó bitrate más bajo.'],
  ...
  const fr: Record<string, string[]> = {
    'tiktok-video-download-no-watermark': ['La vidéo d’origine comporte déjà un filigrane intégré.', 'La variante choisie a basculé vers une version avec filigrane.', 'La publication est passée à un mode de diffusion restreinte.'],
    ...
    'tiktok-video-save-method-ja': ['Un lien court a été copié sans résolution de l’URL finale.', 'La vidéo a été remplacée après une republication modifiée.', 'La publication sélectionnée est un carrousel d’images, pas une vidéo.'],
    'tiktok-video-save-not-working-ja': ['La publication a été supprimée ou est devenue privée.', 'La visibilité du compte a changé après la copie du lien.', 'Une limitation temporaire de la plateforme a bloqué l’extraction.'],
    'download-tiktok-video-hd': ['Aucune variante HD n’est disponible pour cet envoi.', 'La vidéo source était déjà en basse résolution.', 'L’adaptation réseau a choisi un flux à débit plus faible.'],
    ...
    'save-tiktok-video-iphone': ['Le navigateur intégré de l’app a supprimé des paramètres nécessaires.', 'L’iPhone a copié uniquement l’URL d’aperçu.', 'La tâche en arrière-plan a été suspendue par le mode économie d’énergie.'],
    'tiktok-video-download-android': ['L’intent Android a fourni une URL mal formée.', 'Le WebView a bloqué la chaîne de redirection du média.', 'L’optimisation de la batterie a mis la tâche d’extraction en pause.'],
    ...
    'tiktok-video-downloader-free': ['Le point d’accès gratuit a atteint une limite temporaire de quota.', 'Plusieurs tentatives successives ont déclenché une période de refroidissement anti-abus.', 'La route publique du CDN a changé pendant la requête.'],
    'tiktok-video-download-private': ['Le contenu d’un compte privé n’est pas accessible publiquement.', 'Une approbation pour suivre le compte est nécessaire avant l’accès.', 'L’autorisation a changé entre la copie du lien et l’extraction.'],
    ...
    'tiktok-video-downloader-2026': ['Le point d’accès de la plateforme a changé et le cache est devenu obsolète.', 'Une nouvelle limite anti-abus a temporairement bloqué les requêtes.', 'L’ancienne route d’extraction a été abandonnée.'],
    'tiktok-video-link-download': ['Le lien pointe vers un canal ou la page d’accueil, pas vers une publication précise.', 'L’URL courte n’a pas été développée jusqu’à la destination finale.', 'Un fragment de suivi a perturbé la normalisation du parser.'],
  };
  const de: Record<string, string[]> = {
    'tiktok-video-download-no-watermark': ['Das Originalvideo enthält bereits ein eingebranntes Wasserzeichen.', 'Die gewählte Variante ist auf einen Stream mit Wasserzeichen zurückgefallen.', 'Der Beitrag wurde auf eingeschränkte Verbreitung umgestellt.'],
    ...
    'tiktok-video-save-method-ja': ['Es wurde ein Kurzlink kopiert, ohne die endgültige URL aufzulösen.', 'Das Video wurde nach einer bearbeiteten Wiederveröffentlichung ersetzt.', 'Der ausgewählte Beitrag ist ein Bildkarussell und kein Video.'],
    'tiktok-video-save-not-working-ja': ['Der Beitrag wurde entfernt oder auf privat gestellt.', 'Die Sichtbarkeit des Kontos hat sich nach dem Kopieren des Links geändert.', 'Eine temporäre Plattformbegrenzung hat die Extraktion blockiert.'],
    'download-tiktok-video-hd': ['Für diesen Upload ist keine HD-Variante verfügbar.', 'Das Ausgangsvideo hatte bereits eine niedrige Auflösung.', 'Die Netzwerkanpassung hat einen Stream mit niedrigerer Bitrate gewählt.'],
    ...
    'save-tiktok-video-iphone': ['Der In-App-Browser hat erforderliche Parameter entfernt.', 'Auf dem iPhone wurde nur die Vorschau-URL kopiert.', 'Die Hintergrundaufgabe wurde durch den Stromsparmodus angehalten.'],
    'tiktok-video-download-android': ['Der Android-Intent hat eine fehlerhaft formatierte URL geliefert.', 'Die WebView hat die Weiterleitungskette des Mediums blockiert.', 'Die Akkuoptimierung hat die Extraktionsaufgabe pausiert.'],
    ...
    'tiktok-video-downloader-free': ['Der kostenlose Endpunkt hat vorübergehend die Quotenobergrenze erreicht.', 'Mehrere schnelle Wiederholungen haben eine Anti-Missbrauchs-Abkühlphase ausgelöst.', 'Der öffentliche CDN-Pfad hat sich während der Anfrage geändert.'],
    'tiktok-video-download-private': ['Inhalte privater Konten sind nicht öffentlich zugänglich.', 'Vor dem Zugriff auf das Medium ist eine Bestätigung zum Folgen erforderlich.', 'Die Berechtigung hat sich zwischen Kopieren und Extraktion geändert.'],
    ...
    'tiktok-video-downloader-2026': ['Der Plattform-Endpunkt wurde geändert und der Cache ist veraltet.', 'Eine neue Anti-Missbrauchs-Schwelle blockiert Anfragen vorübergehend.', 'Der alte Extraktionspfad wurde eingestellt.'],
    'tiktok-video-link-download': ['Der Link verweist auf einen Kanal oder die Startseite, nicht auf einen konkreten Beitrag.', 'Die Kurz-URL wurde nicht bis zur endgültigen Zieladresse erweitert.', 'Ein Tracking-Fragment hat die Normalisierung des Parsers gestört.'],
  };

  const tr: Record<string, string[]> = {
    'tiktok-video-download-no-watermark': ['Orijinal videoda zaten gömülü bir filigran bulunuyor.', 'Seçilen varyant filigranlı akışa geri düştü.', 'Gönderi kısıtlı dağıtım moduna geçti.'],
    ...
    'tiktok-video-save-method-ja': ['Kısa bağlantı, son URL çözülmeden kopyalandı.', 'Yeniden paylaşım düzenlemesinden sonra video değiştirildi.', 'Seçilen gönderi video değil, görsel karuseli.'],
    'tiktok-video-save-not-working-ja': ['Gönderi kaldırıldı ya da gizliye alındı.', 'Bağlantı kopyalandıktan sonra hesabın görünürlüğü değişti.', 'Geçici platform sınırlaması çıkarma işlemini engelledi.'],
    'download-tiktok-video-hd': ['Bu yükleme için HD varyantı yok.', 'Kaynak video zaten düşük çözünürlüklüydü.', 'Ağ uyarlaması daha düşük bit hızlı akışı seçti.'],
    ...
    'save-tiktok-video-iphone': ['Uygulama içi tarayıcı gerekli parametreleri kaldırdı.', 'iPhone yalnızca önizleme URL’sini kopyaladı.', 'Arka plan görevi düşük güç modu nedeniyle durduruldu.'],
    'tiktok-video-download-android': ['Android intent hatalı biçimlenmiş bir URL verdi.', 'WebView medyanın yönlendirme zincirini engelledi.', 'Pil optimizasyonu çıkarma görevini duraklattı.'],
    ...
    'tiktok-video-downloader-free': ['Ücretsiz uç nokta geçici kota sınırına ulaştı.', 'Arka arkaya denemeler anti-kötüye-kullanım bekleme süresini tetikledi.', 'İstek sırasında genel CDN yolu değişti.'],
    'tiktok-video-download-private': ['Özel hesap içerikleri herkese açık değildir.', 'Medyaya erişmeden önce hesabı takip etmek için onay gerekir.', 'İzin durumu kopyalama ile çıkarma arasında değişti.'],
    ...
    'tiktok-video-downloader-2026': ['Platform uç noktası değişti ve önbellek eskidi.', 'Yeni bir anti-kötüye-kullanım eşiği istekleri geçici olarak engelliyor.', 'Eski çıkarma yolu kullanımdan kaldırıldı.'],
    'tiktok-video-link-download': ['Bağlantı belirli bir gönderiye değil, kanala veya ana sayfaya gidiyor.', 'Kısa URL son hedefe kadar genişletilmedi.', 'Bir takip parçası çözücünün normalizasyonunu bozdu.'],
  };

  const id: Record<string, string[]> = {
    'tiktok-video-download-no-watermark': ['Video asli sudah memiliki watermark bawaan.', 'Varian yang dipilih jatuh ke stream yang masih memakai watermark.', 'Postingan berubah ke mode distribusi terbatas.'],
    ...
    'tiktok-video-save-method-ja': ['Tautan pendek disalin tanpa menyelesaikan URL akhirnya.', 'Video diganti setelah repost yang telah diedit.', 'Postingan yang dipilih adalah carousel gambar, bukan video.'],
    'tiktok-video-save-not-working-ja': ['Postingan dihapus atau diubah menjadi privat.', 'Visibilitas akun berubah setelah tautan disalin.', 'Pembatasan platform sementara memblokir ekstraksi.'],
    'download-tiktok-video-hd': ['Tidak ada varian HD untuk unggahan ini.', 'Video sumber memang sudah beresolusi rendah.', 'Penyesuaian jaringan memilih stream dengan bitrate lebih rendah.'],
    ...
    'save-tiktok-video-iphone': ['Browser di dalam aplikasi menghapus parameter yang dibutuhkan.', 'iPhone hanya menyalin URL pratinjau.', 'Tugas di latar belakang dijeda oleh mode hemat daya.'],
    'tiktok-video-download-android': ['Intent Android menghasilkan URL yang formatnya rusak.', 'WebView memblokir rantai pengalihan medianya.', 'Optimasi baterai menjeda tugas ekstraksi.'],
    ...
    'tiktok-video-downloader-free': ['Endpoint gratis mencapai batas kuota sementara.', 'Percobaan berulang dalam waktu singkat memicu masa pendinginan anti-abuse.', 'Jalur CDN publik berubah saat permintaan berlangsung.'],
    'tiktok-video-download-private': ['Konten dari akun privat tidak bisa diakses secara publik.', 'Persetujuan follow diperlukan sebelum media bisa diakses.', 'Izin berubah di antara penyalinan tautan dan ekstraksi.'],
    ...
    'tiktok-video-downloader-2026': ['Endpoint platform berubah dan cache sudah usang.', 'Ambang anti-abuse yang baru memblokir permintaan untuk sementara.', 'Jalur ekstraksi lama sudah ditinggalkan.'],
    'tiktok-video-link-download': ['Tautan mengarah ke kanal atau beranda, bukan ke postingan tertentu.', 'URL pendek tidak diperluas hingga ke URL media akhir.', 'Fragmen pelacakan merusak normalisasi parser.'],
  };

  const hi: Record<string, string[]> = {
    'tiktok-video-download-no-watermark': ['Original वीडियो में पहले से watermark लगा हुआ है।', 'चुना गया variant watermark वाले stream पर वापस गिर गया।', 'Post restricted distribution mode में चला गया।'],
    ...
    'tiktok-video-save-method-ja': ['Short link कॉपी हुई, लेकिन final URL resolve नहीं हुई।', 'Edited repost के बाद वीडियो बदल दिया गया।', 'चुनी गई post वीडियो नहीं, image carousel है।'],
    'tiktok-video-save-not-working-ja': ['Post delete हो गई या private कर दी गई।', 'Link कॉपी होने के बाद account visibility बदल गई।', 'Temporary platform limit ने extraction रोक दी।'],
    'download-tiktok-video-hd': ['इस upload के लिए HD variant उपलब्ध नहीं है।', 'Source वीडियो पहले से low resolution में था।', 'Network adaptation ने कम bitrate वाला stream चुना।'],
    ...
    'save-tiktok-video-iphone': ['In-app browser ने ज़रूरी parameters हटा दिए।', 'iPhone ने सिर्फ preview URL कॉपी की।', 'Background task low-power mode की वजह से pause हो गई।'],
    'tiktok-video-download-android': ['Android intent ने malformed URL दी।', 'WebView ने media redirect chain block कर दी।', 'Battery optimization ने extraction task रोक दी।'],
    ...
    'tiktok-video-downloader-free': ['Free endpoint temporary quota ceiling तक पहुँच गया।', 'बार-बार retry करने से anti-abuse cooldown trigger हो गया।', 'Request के दौरान public CDN path बदल गया।'],
    'tiktok-video-download-private': ['Private account का content publicly accessible नहीं है।', 'Media access से पहले follow approval ज़रूरी है।', 'Copy और extraction के बीच permission बदल गई।'],
    ...
    'tiktok-video-downloader-2026': ['Platform endpoint बदल गया और cache पुराना हो चुका है।', 'नया anti-abuse threshold temporary requests block कर रहा है।', 'पुराना extraction route अब deprecated है।'],
    'tiktok-video-link-download': ['Link किसी specific post की जगह channel या home पर जा रहा है।', 'Short URL final media URL तक expand नहीं हुई।', 'Tracking fragment ने parser normalization तोड़ दी।'],
  };

  const pt: Record<string, string[]> = {
    'tiktok-video-download-no-watermark': ['O vídeo original já traz a marca d’água incorporada.', 'A variante escolhida caiu em uma versão com marca.', 'A publicação passou para um modo de distribuição restrita.'],
    ...
    'tiktok-video-save-method-ja': ['Foi copiado um link curto sem resolver a URL final.', 'O vídeo foi substituído depois de uma edição de republicação.', 'A publicação selecionada é um carrossel de imagens, não um vídeo.'],
    'tiktok-video-save-not-working-ja': ['A publicação foi removida ou ficou privada.', 'A visibilidade da conta mudou depois da cópia do link.', 'Um limite temporário da plataforma bloqueou a extração.'],
    'download-tiktok-video-hd': ['Não existe variante HD para esse upload.', 'O vídeo de origem ja era de baixa resolução.', 'A adaptacao de banda escolheu um fluxo com bitrate menor.'],
    ...
    'save-tiktok-video-iphone': ['O navegador interno do app removeu parâmetros necessários.', 'O iPhone copiou apenas a URL de visualização.', 'A tarefa em segundo plano foi pausada pelo modo de economia.'],
    'tiktok-video-download-android': ['O intent do Android entregou uma URL malformada.', 'O WebView bloqueou a cadeia de redirecionamento da mídia.', 'A otimização de bateria pausou a tarefa de extração.'],
    ...
    'tiktok-video-downloader-free': ['O endpoint gratuito atingiu um limite temporário de cota.', 'Várias tentativas seguidas ativaram um período de resfriamento antiabuso.', 'A rota pública do CDN mudou durante a requisição.'],
    'tiktok-video-download-private': ['O conteúdo de uma conta privada não fica acessível publicamente.', 'É necessária aprovação para seguir a conta antes do acesso.', 'A permissão mudou entre a cópia do link e a extração.'],
    ...
    'tiktok-video-downloader-2026': ['O endpoint da plataforma mudou e o cache ficou defasado.', 'Um novo limite antiabuso bloqueou as requisições temporariamente.', 'A rota antiga de extração foi descontinuada.'],
    'tiktok-video-link-download': ['O link aponta para um canal ou para a página inicial, não para uma publicação específica.', 'A URL curta não foi expandida até o destino final.', 'Um fragmento de rastreamento atrapalhou a normalização do parser.'],
  };
...
function comparisonSpecificFailLines(locale: BlogLocale, slug: string): string[] | null {
  ...
  const ja: Record<string, string[]> = {
    'best-twitter-video-downloader-2026': ['比較時に古いツール版を使っていました。', '信頼性を判断する基準が明確ではありませんでした。', '各ツールで同じテスト用リンクを使っていませんでした。'],
    ...
    'safest-video-downloader-sites': ['評価時点でポリシーページが用意されていませんでした。', 'ドメイン信頼性の確認が行われていませんでした。', '広告挙動のリスクが十分に加味されていませんでした。'],
    'twitter-video-downloader-not-working': ['原因分析でネットワーク要因とプラットフォーム要因が混ざっていました。', 'トラブルシュート手順に再試行ポリシーがありませんでした。', '無効な投稿 URL のまま検証していました。'],
    'tiktok-downloader-not-working': ['公開投稿と非公開投稿を分けて見ていませんでした。', 'rate limit の症状を不具合と誤認していました。', 'ローカルのブラウザ拡張を切り分けていませんでした。'],
    'telegram-video-download-not-working': ['非公開チャットのリンクを公開リンクのように扱っていました。', 'チャンネルのアクセス条件を考慮していませんでした。', 'CDN の timeout と権限エラーをひとまとめにしていました。'],
    'is-video-downloader-legal': ['法域ごとの差を切り分けていませんでした。', '個人保存と再配布を同じ前提で扱っていました。', 'プラットフォーム規約を利用ケースに結び付けていませんでした。'],
    'video-downloader-safe-or-not': ['安全性の基準にスクリプトの出所確認がありませんでした。', 'malware / adware の評判確認が行われていませんでした。', 'チェックリストに HTTPS/TLS の確認が入っていませんでした。'],
    'free-video-downloader-online-comparison': ['無料プランの制限を同じ条件で比較していませんでした。', '広告量の影響を測定していませんでした。', '速度比較がばらばらなネットワーク環境で行われていました。'],
    'top-5-video-downloader-tools': ['ランキング要素が定義されていませんでした。', 'レビュー途中で配点の重みが変わっていました。', '再現できるテスト手順が示されていませんでした。'],
  };

  const es: Record<string, string[]> = {
    'best-twitter-video-downloader-2026': ['Se compararon versiones desactualizadas.', 'Faltaron critérios claros de fiabilidad.', 'No se usaron enlaces idénticos para pruebas.'],
  ...
  const fr: Record<string, string[]> = {
    'best-twitter-video-downloader-2026': ['La comparaison s’est appuyée sur des versions d’outils dépassées.', 'Aucun critère clair de fiabilité n’a été appliqué.', 'Les mêmes liens de test n’ont pas été utilisés sur tous les outils.'],
    ...
    'safest-video-downloader-sites': ['Des pages de politique manquaient au moment de l’évaluation.', 'Aucune vérification de confiance du domaine n’a été effectuée.', 'Le risque lié au comportement publicitaire n’a pas été assez pondéré.'],
    'twitter-video-downloader-not-working': ['La cause racine a mélangé un problème réseau et un problème de plateforme.', 'Aucune politique de nouvelle tentative n’était prévue dans le flux de résolution.', 'Des liens de publication invalides ont été utilisés pendant les tests.'],
    ...
    'telegram-video-download-not-working': ['Des liens de chat privé ont été testés comme s’ils étaient publics.', 'Les règles d’accès du canal ont été ignorées.', 'Les erreurs de timeout du CDN et les erreurs de permission ont été mélangées.'],
    'is-video-downloader-legal': ['Les différences de juridiction n’ont pas été séparées.', 'L’archive personnelle et la redistribution ont été traitées comme la même chose.', 'Les conditions de la plateforme n’ont pas été reliées au cas d’usage.'],
    'video-downloader-safe-or-not': ['Les critères de sécurité n’ont pas vérifié la provenance des scripts.', 'Aucune vérification de réputation contre les malwares ou adwares n’a été faite.', 'La liste ne comprenait pas de validation HTTPS/TLS.'],
    'free-video-downloader-online-comparison': ['Les limites des versions gratuites n’ont pas été normalisées.', 'L’impact de la charge publicitaire n’a pas été mesuré.', 'Les tests de vitesse ont été menés sur des réseaux incohérents.'],
    'top-5-video-downloader-tools': ['Les critères de classement n’étaient pas définis.', 'Les pondérations de score ont changé pendant l’analyse.', 'Aucun protocole de test reproductible n’a été présenté.'],
  };
  const de: Record<string, string[]> = {
    'best-twitter-video-downloader-2026': ['Beim Vergleich wurden veraltete Tool-Versionen verwendet.', 'Es gab kein klares Kriterium für Zuverlässigkeit.', 'Für die Tests wurden nicht überall identische Links benutzt.'],
    ...
    'safest-video-downloader-sites': ['Zum Zeitpunkt der Bewertung fehlten Richtlinienseiten.', 'Es wurde keine Vertrauensprüfung der Domain durchgeführt.', 'Das Risiko durch Werbeverhalten wurde nicht ausreichend gewichtet.'],
    'twitter-video-downloader-not-working': ['Die Ursachenanalyse hat Netzwerk- und Plattformfehler vermischt.', 'Im Lösungsablauf gab es keine klare Retry-Strategie.', 'In den Tests wurden ungültige Beitragslinks verwendet.'],
    ...
    'telegram-video-download-not-working': ['Private Chat-Links wurden so getestet, als wären sie öffentlich.', 'Die Zugriffsregeln des Kanals wurden ignoriert.', 'CDN-Timeouts und Berechtigungsfehler wurden zusammengeworfen.'],
    'is-video-downloader-legal': ['Unterschiede zwischen Rechtsräumen wurden nicht getrennt betrachtet.', 'Persönliches Archiv und Weiterverbreitung wurden vermischt.', 'Die Plattformregeln wurden nicht auf den konkreten Anwendungsfall bezogen.'],
    'video-downloader-safe-or-not': ['Die Sicherheitskriterien haben die Herkunft der Skripte nicht geprüft.', 'Es gab keine Reputationsprüfung auf Malware oder Adware.', 'In der Checkliste fehlte eine HTTPS/TLS-Prüfung.'],
    'free-video-downloader-online-comparison': ['Die Grenzen kostenloser Tarife wurden nicht normalisiert.', 'Die Auswirkungen der Werbelast wurden nicht gemessen.', 'Die Geschwindigkeitstests liefen auf uneinheitlichen Netzwerken.'],
    'top-5-video-downloader-tools': ['Die Ranking-Faktoren waren nicht klar definiert.', 'Die Gewichtung der Punkte änderte sich während der Auswertung.', 'Es wurde kein reproduzierbares Testprotokoll vorgelegt.'],
  };

  const tr: Record<string, string[]> = {
    'best-twitter-video-downloader-2026': ['Karşılaştırmada araçların güncel olmayan sürümleri kullanıldı.', 'Güvenilirlik için net bir ölçüt uygulanmadı.', 'Tüm araçlarda aynı test bağlantıları kullanılmadı.'],
    ...
    'safest-video-downloader-sites': ['Değerlendirme sırasında politika sayfaları eksikti.', 'Alan adı güven kontrolü yapılmadı.', 'Reklam davranışı riski yeterince ağırlıklandırılmadı.'],
    'twitter-video-downloader-not-working': ['Kök neden analizi ağ ve platform hatalarını birbirine karıştırdı.', 'Sorun giderme akışında yeniden deneme politikası yoktu.', 'Testlerde geçersiz gönderi bağlantıları kullanıldı.'],
    ...
    'telegram-video-download-not-working': ['Özel sohbet bağlantıları herkese açıkmış gibi test edildi.', 'Kanal erişim kuralları göz ardı edildi.', 'CDN zaman aşımı ile izin hataları aynı başlık altında toplandı.'],
    'is-video-downloader-legal': ['Yargı alanı farkları ayrıştırılmadı.', 'Kişisel arşiv ile yeniden dağıtım aynı şey gibi ele alındı.', 'Platform şartları kullanım senaryosuna eşlenmedi.'],
    'video-downloader-safe-or-not': ['Güvenlik ölçütleri betik kaynağını doğrulamadı.', 'Kötü amaçlı yazılım veya reklam yazılımı itibarı kontrol edilmedi.', 'Kontrol listesinde HTTPS/TLS doğrulaması yoktu.'],
    'free-video-downloader-online-comparison': ['Ücretsiz katman sınırları normalleştirilmedi.', 'Reklam yükünün etkisi ölçülmedi.', 'Hız testleri tutarsız ağlarda yapıldı.'],
    'top-5-video-downloader-tools': ['Sıralama ölçütleri tanımlanmadı.', 'Puan ağırlıkları inceleme sırasında değişti.', 'Tekrarlanabilir bir test protokolü sunulmadı.'],
  };

  const id: Record<string, string[]> = {
    'best-twitter-video-downloader-2026': ['Perbandingan memakai versi alat yang sudah usang.', 'Tidak ada kriteria keandalan yang jelas.', 'Tautan uji yang dipakai tidak konsisten di semua alat.'],
    ...
    'safest-video-downloader-sites': ['Halaman kebijakan tidak tersedia saat evaluasi dilakukan.', 'Tidak ada pemeriksaan kepercayaan domain.', 'Risiko dari perilaku iklan tidak diberi bobot yang cukup.'],
    'twitter-video-downloader-not-working': ['Akar masalah mencampur error jaringan dan error platform.', 'Tidak ada kebijakan retry di langkah pemecahan masalah.', 'Pengguna menguji dengan URL tweet yang tidak valid.'],
    ...
    'telegram-video-download-not-working': ['Tautan chat privat diuji seolah-olah publik.', 'Aturan akses kanal diabaikan.', 'Error timeout CDN dan error izin digabungkan.'],
    'is-video-downloader-legal': ['Perbedaan yurisdiksi tidak dipisahkan.', 'Arsip pribadi dan redistribusi diperlakukan sebagai hal yang sama.', 'Ketentuan platform tidak dipetakan ke kasus penggunaan.'],
    'video-downloader-safe-or-not': ['Kriteria keamanan tidak memeriksa asal skrip.', 'Tidak ada pemeriksaan reputasi malware atau adware.', 'Tidak ada validasi HTTPS/TLS di daftar periksa.'],
    'free-video-downloader-online-comparison': ['Batas paket gratis tidak dinormalisasi.', 'Dampak beban iklan tidak diukur.', 'Uji kecepatan dijalankan pada jaringan yang tidak konsisten.'],
    'top-5-video-downloader-tools': ['Faktor peringkat tidak didefinisikan.', 'Bobot penilaian berubah selama ulasan berlangsung.', 'Tidak ada protokol pengujian yang bisa direproduksi.'],
  };

  const hi: Record<string, string[]> = {
    'best-twitter-video-downloader-2026': ['Comparison में tools के पुराने versions इस्तेमाल किए गए।', 'Reliability के लिए कोई स्पष्ट मानदंड लागू नहीं किया गया।', 'हर tool पर एक जैसे test links इस्तेमाल नहीं हुए।'],
    ...
    'safest-video-downloader-sites': ['Evaluation के समय policy pages उपलब्ध नहीं थीं।', 'Domain trust check नहीं किया गया।', 'Ad behavior risk को पर्याप्त weight नहीं दिया गया।'],
    'twitter-video-downloader-not-working': ['Root cause analysis में network और platform errors मिल गए।', 'Troubleshooting steps में retry policy नहीं थी।', 'Users ने invalid tweet URLs के साथ test किया।'],
    ...
    'telegram-video-download-not-working': ['Private chat links को public मानकर test किया गया।', 'Channel access rules को नज़रअंदाज़ किया गया।', 'CDN timeout और permission errors को एक साथ मिला दिया गया।'],
    'is-video-downloader-legal': ['Jurisdiction के फर्क अलग नहीं किए गए।', 'Personal archive और redistribution को एक ही चीज़ माना गया।', 'Platform ToS को use case से नहीं जोड़ा गया।'],
    'video-downloader-safe-or-not': ['Security criteria में script provenance की जाँच नहीं थी।', 'Malware या adware reputation checks नहीं किए गए।', 'Checklist में HTTPS/TLS validation शामिल नहीं थी।'],
    'free-video-downloader-online-comparison': ['Free-tier limits को normalize नहीं किया गया।', 'Ad load के प्रभाव को measure नहीं किया गया।', 'Speed tests असंगत networks पर चलाए गए।'],
    'top-5-video-downloader-tools': ['Ranking factors स्पष्ट रूप से परिभाषित नहीं थे।', 'Review के दौरान scoring weights बदलती रहीं।', 'कोई reproducible test protocol पेश नहीं किया गया।'],
  };

  const pt: Record<string, string[]> = {
    'best-twitter-video-downloader-2026': ['A comparação usou versões desatualizadas das ferramentas.', 'Não houve um critério claro de confiabilidade.', 'Os mesmos links de teste não foram usados em todas as ferramentas.'],
    ...
    'safest-video-downloader-sites': ['Faltavam páginas de política no momento da avaliação.', 'Não foi feita uma checagem de confiança do domínio.', 'O risco do comportamento publicitário não recebeu peso suficiente.'],
    'twitter-video-downloader-not-working': ['A causa raiz misturou erro de rede com erro da plataforma.', 'Não havia política de nova tentativa no fluxo de solução.', 'Foram usados links de publicação inválidos nos testes.'],
    ...
    'telegram-video-download-not-working': ['Links de chat privado foram testados como se fossem públicos.', 'As regras de acesso do canal foram ignoradas.', 'Erros de timeout do CDN e erros de permissão foram misturados.'],
    'is-video-downloader-legal': ['As diferenças de jurisdição não foram separadas.', 'Arquivo pessoal e redistribuição foram tratados como a mesma coisa.', 'Os termos da plataforma não foram ligados ao caso de uso.'],
    'video-downloader-safe-or-not': ['Os critérios de segurança não verificaram a procedência dos scripts.', 'Não houve checagem de reputação para malware ou adware.', 'A lista não incluía validação de HTTPS/TLS.'],
    'free-video-downloader-online-comparison': ['Os limites das versões gratuitas não foram normalizados.', 'O impacto da carga de anúncios não foi medido.', 'Os testes de velocidade foram feitos em redes inconsistentes.'],
    'top-5-video-downloader-tools': ['Os critérios de ranking não estavam definidos.', 'Os pesos de pontuação mudaram durante a análise.', 'Não foi apresentado um protocolo de teste reproduzível.'],
  };
...
function articleFaqAnswer(locale: BlogLocale, slug: string, category: string): string {
  ...
  if (category === 'tiktok') {
    ...
    if (locale === 'es') return `Para ${slug} en TikTok, usa la URL directa de la publicación y confirma que el vídeo sea público antes de reintentar.`;

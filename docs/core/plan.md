...
- 10言語対応を前提に、多言語 UI / metadata / SEO 基盤を維持する。

### 1.2 主な機能
- SNS 別メディア抽出導線。
- 問題解決ページ（"動かない" / "保存できない" 等）による解決導線。
- 非ログイン・高負荷時の degraded モード。
- test / prod を分離した安全なデプロイ運用。

### 1.3 MVP (最小実用製品) のスコープ
- Telegram / X の基本抽出フロー。
- 主要な問題解決ページ一式。
- RTL レイアウト対応を含む多言語 UI 基盤。
- Cloudflare Workers / D1 / OpenNext を前提にした配信基盤。
...
- リリース時は `check:release:test` / `check:release:prod` を通してからデプロイする。
...
- 失敗時は原因別の解決導線へ遷移させる。
- 優先改善順は Telegram / X / TikTok を軸としつつ、他プラットフォームは個別仕様書で安定化管理する。
...
- バックエンド: Next.js Route Handlers / Cloudflare Workers
- データ基盤: Cloudflare D1
- リアルタイム/状態管理: Cloudflare Durable Objects
- 配信: Cloudflare Workers + OpenNext

## 8. デプロイ運用
- 本番: `wrangler.production.toml`
- テスト: `wrangler.test.toml`
- `wrangler.toml` を手動で切り替えない。

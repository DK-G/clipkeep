...
> **2026-07-10 実地結果（重要）**: 実際に試したところ、冷たい新規登録ルートは軒並みゲートで詰まった。
> これは偶然でなく、**新規アカウントの宣伝リンク投下を止めるための各サイトのアンチスパム設計**。
> - **① Show HN**: 新規アカは Show HN 不可（`/showlim`。karma 蓄積＝継続手作業が前提）。ユーザー方針に反する→**見送り**。
> - **② AlternativeTo**: 新規登録は**アカウント7日経過が必要**（"try again after July 17, 2026"）。ただし
>   **クローズド/ホスト型ツールは掲載可＝ClipKeep は正当な適合**。→ **7/17 に1回やるだけ**（"一度きり"に合致）。
> - **⑤ awesome リスト（awesome-video-downloaders 等）**: **OSS 優先・GitHub リポジトリ URL 前提・広告型は非掲載**
>   ＝ClipKeep（クローズド/広告/公開リポなし）は**適合せず→押し込みは §6 抵触なので行わない**。
> - 結論: **"ゼロから冷たく登録して回る"手法は構造的に弾かれる**。有効なのは (a) 適合する②を待って1回、
>   (b) アセット自体で自然リンク/バッジ埋め込みを稼ぐ（自律）、(c) ユーザーが本当に参加したい所だけ地道に。
>

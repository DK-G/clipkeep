...
- 格納方式: **KV（新規バインディング `TREND_KV`）を採用（2026-06-21 調査で確定）**。
  当初「ビルド時 JSON を優先」としていたが、**トピックは cron（ランタイム）が動的に発見するため静的なビルド時 JSON には
  書き込めない**（ランタイム write → SSR ランタイム read が必要）。`wrangler.production.toml` に既存 KV は無いため、
  D1 スキーマを足さない範囲での適合策は KV のみ。判断5の「新規 KV バインディングは承認範囲内」に基づき `TREND_KV` を新設する
  （D1 スキーマ・既存 bindings には触れない）。`wrangler.production.toml` と `wrangler.test.toml` 双方に宣言。
...
5. **Phase 0 格納方式**: KV（新規バインディング `TREND_KV`）。D1 スキーマ非依存。
   ※調査の結果ビルド時 JSON はランタイムのトピック発見を書き込めず不適合と判明したため、KV に確定（§6 Phase 0 参照）。

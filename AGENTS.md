...
- Known risks: OpenNext build/deploy settings are sensitive; rate-limit thresholds affect extraction UX; analytics exports in `docs/analytics/` are review inputs, not automatic product decisions; `.env`, `.env.local`, `.env.analytics.local`, `.wrangler`, `.next`, and `.open-next` must remain local artifacts. **Credentials (GA4/GSC service-account key, OAuth client) live outside the repo in `D:\secrets\clipkeep\`** — nothing secret goes under `D:\dev` (`D:\dev\docs\SAFETY_CHECKLIST.md` Red Lines). `.env.analytics.local` holds only the path to them; see `docs/ops/analytics-auth-recovery.md` §4.2.

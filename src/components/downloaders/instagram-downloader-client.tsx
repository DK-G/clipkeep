...
import { instagramText, Locale, localeDir } from '@/lib/i18n/ui';
import { AdsterraNative } from '@/components/ads/native-banner';
import { GallerySection } from '@/components/gallery-section';

interface InstagramDownloaderClientProps {
  locale: Locale;
}
    ...
    <main dir={dir} className="max-w-4xl mx-auto px-6 py-12">
      ...
      <div className="grid md:grid-cols-2 gap-12 text-gray-700 dark:text-slate-300">
        <section className="bg-white dark:bg-slate-900 p-8 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm">
          <h2 className="text-2xl font-bold mb-6 pb-2 border-b-2 border-pink-50 dark:border-pink-900/30 text-gray-900 dark:text-slate-100">{t.howToTitle}</h2>
        ...
        <section className="bg-white dark:bg-slate-900 p-8 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm">
          <h2 className="text-2xl font-bold mb-6 pb-2 border-b-2 border-pink-50 dark:border-pink-900/30 text-gray-900 dark:text-slate-100">{t.whyTitle}</h2>
      ...
      {t.trendingTitle && (
        <GallerySection id="trending" platform="instagram" locale={locale} title={t.trendingTitle} type="trending" />
      )}

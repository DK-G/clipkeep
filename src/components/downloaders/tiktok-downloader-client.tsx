...
interface TikTokDownloaderClientProps {
  locale: Locale;
}

export function TikTokDownloaderClient({ locale }: TikTokDownloaderClientProps) {
  ...
  return (
    <main dir={dir} className="max-w-4xl mx-auto px-6 py-12">
      ...
      {t.note && (
        <div className="mb-12 p-6 bg-amber-50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/30 rounded-2xl">
          ...
          <p className="text-amber-700/80 dark:text-amber-300/80 text-sm leading-relaxed">{t.noteBody}</p>
        </div>
      )}
      ...
      <GallerySection id="realtime" platform="tiktok" locale={locale} title="Real-time / リアルタイム" type="recent" />
      ...
      {t.trendingTitle && (
        <GallerySection id="trending" platform="tiktok" locale={locale} title={t.trendingTitle} type="trending" />
      )}

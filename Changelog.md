...
## 2026-03-22（CL-0061） トレンド・最新ページのレイアウト一致と翻訳の修正

### 概要
- トレンド(Trend)と最新(Latest)の専用ページを、ホームページの各セクションと同じタイルレイアウト（Masonry）に統一。
- `GallerySection`を拡張し、トレンドページで「高密度（dense）表示」と「メタ情報（再生時間・アクセス数）の非表示」を実現。

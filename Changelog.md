...
## 2026-06-08 path-based hreflang の追加
...
- `task.md` / `diff.md`: Stage2 作業結果へ更新。

**確認方法**:
1. 人間環境で `npm run lint` を実行する。
2. 人間環境で `npm run typecheck` を実行する。
3. 人間環境で `npm run build` を実行する。
4. `/ja/faq`, `/pt/trending`, `/ar/solution/<slug>` の canonical / alternates / sitemap を確認する。
...
## 2026-03-28（CL-0066） 「デモを試す」機能の完全復旧（演出の復元と静的成功データの導入）

### 概要
- **デモ用「静的成功データ」の導入**: 環境依存の抽出失敗（IPブロック等）を回避するため、デモ専用 Job ID (`job_tiktok_7c1de28c9edb807c`) に対してサーバー側で成功レスポンスを固定（Pinned Response）するインターベンションを実装。これにより、100% 確実にデモが成功する状態を確保しました。
- **バックエンド bypass 実装**: デモ専用 URL に限り、サーバー側で Turnstile 認証をスキップするように `prepare` API を改修しました。
- **読み込み演出（演出）の復元**: フロントエンドの `ExtractorForm` において、デモクリック時も通常と同様の読み込みアニメーション（スピナーとステータス表示）を表示するように戻しました。
- **欠落アセットの追加**: 404 エラーの原因となっていた `placeholder-video.png` を新規生成し、`public` ディレクトリに追加しました。
- **シームレスな体験**: ユーザーが認証を意識することなく、ワンクリックでデモから抽出結果のダウンロードまでを確実に体験できるフローを再構築しました。
